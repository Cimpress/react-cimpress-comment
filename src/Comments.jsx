...
export default class _Comments extends React.Component {
  ...
  constructor(props) {
    ...
    this.customizrResource = encodeURIComponent(`https://comment.trdlnk.cimpress.io/`)
  ...
  render() {
    ...
    if (!this.props.resourceUri) {
      ...
      comments = this.state.commentsIds.map(
        (id, index) => <Comment className={'comment ' + ((index % 2 === 0) ? 'comment-even' : 'comment-odd')} key={id}
    ...
    let addCommentBox = <div>
      <div
        ...
        <Alert
          type={"info"}
          message={<p>Use <strong>@</strong> character to mention people. We will notify them via email about this
            comment thread.</p>}
          dismissible={true}
